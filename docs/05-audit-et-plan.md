...
### Habillage charbon et accent unique (référence « Vaulto »)

Le violet et son dégradé magenta–cyan cèdent la place à un thème charbon à accent
orange, calqué sur la maquette fournie.

- **Surfaces neutres, par paliers.** Fond `#0c0c0c`, carte `#151515`, carte surélevée
  `#1e1e1e`, filets blancs à 8 %. Plus aucune surface teintée : un fond coloré rogne le
  contraste des séries, seules porteuses d'information.
- **Une teinte d'action, et une seule.** L'orange `#ff4a00` dit « c'est ici qu'on
  clique » — boutons pleins en gélule, curseur actif, focus. Il n'encode jamais une
  donnée. Les états (bon, à surveiller, critique) gardent leurs couleurs réservées.
- **L'entrée de menu active** est une pastille plus claire cernée d'un filet, comme la
  référence, plutôt qu'un aplat coloré : la couleur reste au service de l'action.
- **Plus de translucidité ni de flou.** Cartes et fenêtres sont opaques ; les halos du
  fond tombent à une trace d'orange en haut à gauche. Un décor moins bavard, et quelques
  images de moins à composer à chaque défilement.
- **Les anneaux du tableau de bord** portent tous la même teinte. Chacun est seul dans sa
  carte et porte son titre : quatre couleurs suggéreraient quatre familles là où il n'y
  en a pas.

**La palette de données, elle, a été revalidée, pas redécorée.** Les huit teintes de
série sont repassées au validateur contre les nouvelles surfaces réelles :

- en sombre (`#151515`, `#1e1e1e`) : tout passe sans retouche ;
- en clair, les cartes devenues **blanc pur**, trois pas tombaient sous 3:1 — vert d'eau,
  or clair, rose clair. Ils ont été assombris (`#0e9b6b`, `#b07000`, `#d55181`) et
  l'orange de série avec eux (`#d9531f`), pour préserver la séparation sous vision
  protanope. Les cinq contrôles calculés repassent au vert sur `#ffffff`, `#fafafa` et
  `#f4f4f3`.

Une part de camembert qu'on ne distingue pas du fond ne dit rien, quelle que soit
l'élégance du thème autour.
